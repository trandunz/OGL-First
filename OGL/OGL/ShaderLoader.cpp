...
GLuint ShaderLoader::CreateGeometryProgram(const char* VertexShaderFilename, const char* FragmentShaderFilename, const char* GeometryShaderFileName)
{

	// Create the shaders from the filepath
	GLuint vertexShaderID = CreateShader(GL_VERTEX_SHADER, VertexShaderFilename);
	GLuint fragmentShaderID = CreateShader(GL_FRAGMENT_SHADER, FragmentShaderFilename);
	GLuint geometryShaderID = CreateShader(GL_GEOMETRY_SHADER, GeometryShaderFileName);
	...
	if (link_result == GL_FALSE)
	{
		std::string programName = GeometryShaderFileName + *FragmentShaderFilename;
		PrintErrorDetails(false, program, programName.c_str());
		return 0;
	}
	return program;
